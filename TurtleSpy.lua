...
if not isfile("OrbitSpySettings.json") then
    writefile("OrbitSpySettings.json", HttpService:JSONEncode(settings))
else
    if HttpService:JSONDecode(readfile("OrbitSpySettings.json"))["Main"] then
        writefile("OrbitSpySettings.json", HttpService:JSONEncode(settings))
    else
        settings = HttpService:JSONDecode(readfile("OrbitSpySettings.json"))
    end
end
...
if game.CoreGui:FindFirstChild("OrbitSpy") then
    game.CoreGui.OrbitSpy:Destroy()
end
...
local OrbitSpy = Instance.new("ScreenGui")
...
OrbitSpy.Name = "OrbitSpy"

Parent(OrbitSpy)

mainFrame.Name = "mainFrame"
mainFrame.Parent = OrbitSpy
...
mouse.KeyDown:Connect(function(key)
    if key:lower() == settings["Keybind"]:lower() then
        OrbitSpy.Enabled = not OrbitSpy.Enabled
    end
end)
...
Code.Text = "Thanks for Using our 0rbitSpy! :)"
...
CodeComment.Text = "-- Script generated by 0rbitSpy!"
